...
const handleSignOut = () => {
    ...
    return (
        ...
        <div className="flex-1 flex justify-center mx-auto ml-48">
            <div className="hidden md:flex items-baseline space-x-4 ml-0 ">
                <NavLink to="/">
        ...
        {/* Responsive menu */}
        <div className={`${isOpen ? 'block' : 'hidden'} md:hidden fixed top-16 right-2 bg-white bg-opacity-30 backdrop-blur-md p-4 rounded-lg shadow-lg mr-56`}>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <motion.div initial={{ y: -50 }} animate={{ x: 0, y: -1, scale: 1.2 }} transition={{ duration: 0.5, type: 'spring', stiffness: 70 }} className="w-12 rounded-full btn m-1">
                                ...
                                {
                                    // condition ? it (or:) it
                                    user?.email ? <img alt="user photo" className='size-10 rounded-full' src={user?.photoURL}></img> : 
                                    <SpreadingCircleLoader className='size-9 border rounded-full shadow-2xl border-teal-500 '></SpreadingCircleLoader>
                                }
                            </motion.div>
                        </DropdownMenuTrigger>
